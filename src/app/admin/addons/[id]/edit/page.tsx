...
export default function EditAddOn() {
  ...
  const fetchAddOn = async () => {
    try {
      const response = await axios.get(`/api/addons/${params.id}`)
      const addOn = response.data
      setName(addOn.name || '')
      setDescription(addOn.description || '')
      setPrice(addOn.price?.toString() || '')
      setUnit(addOn.unit || 'หน่วย')
      setIsActive(addOn.isActive !== undefined ? addOn.isActive : true)
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/addons/${params.id}`, {
        name: name.trim(),
      ...
      toast.error(error.response?.data?.error || 'ไม่สามารถแก้ไขอ๊อฟชั่นเสริมได้')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">แก้ไขอ๊อฟชั่นเสริม</h1>
            <p className="text-gray-700 text-lg">แก้ไขข้อมูลอ๊อฟชั่นเสริม</p>
          </div>
        </div>
