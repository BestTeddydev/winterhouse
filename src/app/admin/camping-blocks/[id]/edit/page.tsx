...
export default function EditCampingBlock() {
  ...
  const fetchBlock = async () => {
    try {
      const response = await axios.get(`/api/camping-blocks/${params.id}`)
      ...
      if (block.imageUrls && block.imageUrls.length > 0) {
        const validUrls = block.imageUrls.filter((url: string) => url && url.trim() !== '' && !url.includes('placeholder'))
      ...
      setPricePerPerson(block.pricePerPerson?.toString() || '')
      setMaxCapacity(block.maxCapacity?.toString() || '')
      setMinCapacity(block.minCapacity?.toString() || '1')
      setAmenities(block.amenities || [])
      setIsActive(block.isActive !== undefined ? block.isActive : true)
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`/api/camping-blocks/${params.id}`, blockData)
      toast.success('อัปเดตบล็อคกางเต๊นท์สำเร็จ')
      router.push('/admin/camping-blocks')
    } catch (error: any) {
      console.error('Error updating camping block:', error)
      toast.error(error.response?.data?.error || 'ไม่สามารถอัปเดตบล็อคกางเต๊นท์ได้')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">แก้ไขบล็อคกางเต๊นท์</h1>
